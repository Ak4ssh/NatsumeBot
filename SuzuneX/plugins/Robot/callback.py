...
from Suzunex import YouTube, app
from Suzunex.core.call import TheVenomXD
from Suzunex.misc import SUDOERS, db
from Suzunex.utils import bot_sys_stats
from Suzunex.utils.database import (is_active_chat,
...
from Suzunex.utils.formatters import seconds_to_min
from Suzunex.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup,
                                          close_keyboard)
from Suzunex.utils.stream.autoclear import auto_clean
from Suzunex.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await TheVenomXD.pause_stream(chat_id)
        ...
        await CallbackQuery.answer()
        await TheVenomXD.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await TheVenomXD.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await TheVenomXD.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await TheVenomXD.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
